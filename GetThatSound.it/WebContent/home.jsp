...
<body class="homepage is-preload">
		...
		<nav id="nav">
			<ul>
				...
				<li><a href="/admin/admin.jsp">Account</a>
					<ul>
						<li><a href="/admin/admin.jsp#listaUtenti">Gestisci utenti</a></li>
						<li><a href="/admin/admin.jsp#creaManager">Crea un manager</a></li>
						<li><a href="logout">Esci</a></li>
					</ul>
				</li>
				<li><a>Ciao <%=cb.getNome()%> =)</a></li>
				...
				<header>
					...
					<h6>
						<form name="Search" action="/result.jsp" style="display:-webkit-inline-flex" onsubmit="return SearchFormValidation();">
							...
							<button id="avviaRicerca" type="submit" style="margin-top: 1em;margin-bottom: 1em"><i class="fa fa-search"></i></button>		
						</form>
					</h6>
				</header>
			</article>
	...
	//tale funzione (playSong) pesca i campi del brano di indice i e li carica nel player e fa partire il play	
		...
		$(document).ready(
				function() {
					$.getJSON("/GetHomeBrani", function(data, status) {
						var i = 0;			 
						var out = "<article><header><table class=\"branoTable\">";
						while (data[i] != null) {
							var id = data[i].id;
							out+="<tr id=\"brano\">" 
								+"<th><a href=\"/profilo.jsp?utente="+data[i].user_id+"\"><img style=\"width:100%\" src=\"/images/cover.jpg\"></a></th>"
								+"<th id=\"titolo"+id+"\">" + data[i].titolo + "</th>" 
								+"<th id=\"album"+id+"\">" + data[i].album + "</th>" 
								+"<th id=\"artista"+id+"\">" + data[i].artista + "</th>" 
								+"<th><button name=\"riproduci\" id="+(data[i].path)+" value="+ id +" type=\"button\" class=\"ss-icon\" onClick=\"playSong(this.id, this.value);\">Play</button></th>" 
